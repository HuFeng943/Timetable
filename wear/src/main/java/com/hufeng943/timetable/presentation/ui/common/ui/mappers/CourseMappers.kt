...
fun Course.toCourseUi(timeSlot: TimeSlot): CourseUi = CourseUi(
    id = this.id,
    name = name,
    timeSlot = timeSlot.toTimeSlotUi(),
    color = if (this.color == -1L) Color.Unspecified else Color(this.color),
    location = this.location,
    teacher = this.teacher
)

fun Course.toCourseEditUi(): CourseEditUi = CourseEditUi(
    id = id,
    name = name,
    timeSlots = timeSlots.map { it.toTimeSlotUi() },
    location = location,
    color = if (color == -1L) Color.Unspecified else Color(color),
    teacher = teacher
)

fun CourseEditUi.toCourse(): Course = Course(
    id = id,
    name = name,
    timeSlots = emptyList(), // Slots are usually updated separately in this app's architecture
    location = location,
